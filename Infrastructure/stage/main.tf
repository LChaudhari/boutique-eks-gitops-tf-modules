...
# ---------------------------------------------------------------------------
# 1. Networking (foundation — no dependencies)
# ---------------------------------------------------------------------------
module "vpc" {
  source = "../modules/vpc"
...
# ---------------------------------------------------------------------------
# 2. ECR repositories (independent — no dependencies)
# ---------------------------------------------------------------------------
module "ecr" {
  source = "../modules/ecr"
...
# ---------------------------------------------------------------------------
# 3. DNS + TLS certificate (independent; only created when enable_dns = true)
#    existing Route53 zone; ACM cert + validation records
# ---------------------------------------------------------------------------
module "dns" {
  source = "../modules/dns"
...
# ---------------------------------------------------------------------------
# 4. EKS cluster, nodes, IRSA, managed add-ons (depends on: vpc)
# ---------------------------------------------------------------------------
module "eks" {
  source = "../modules/eks"
...
# These read module.eks outputs, so they are only usable once eks exists.
# ---------------------------------------------------------------------------
provider "kubernetes" {
  alias                  = "eks"
...
# ---------------------------------------------------------------------------
# 5. Platform add-ons: ALB controller, ExternalDNS, metrics-server, gp3 SC
#    (depends on: eks)
# ---------------------------------------------------------------------------
module "addons" {
...
# ---------------------------------------------------------------------------
# 6. ArgoCD (GitOps) + kube-prometheus-stack, exposed via ALB Ingress
#    (depends on: addons)
# ---------------------------------------------------------------------------
module "argocd" {
